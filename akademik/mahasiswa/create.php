...
if (!$conn) {
    die("Koneksi database gagal di create.php: " . mysqli_connect_error());
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validasi dasar
    if (empty($nim) || empty($nama) || empty($alamat) || empty($tgl)) {
        ...
        $sql_insert = "INSERT INTO mahasiswa (nim, nama, alamat, tanggal_lahir) VALUES (?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conn, $sql_insert)) {
            mysqli_stmt_bind_param($stmt, "ssss", $nim, $nama, $alamat, $tgl); // 'ssss' karena semua adalah string
            if (mysqli_stmt_execute($stmt)) {
                header("Location: index.php");
                exit(); // Penting untuk menghentikan eksekusi script setelah header
            } else {
                echo "<div class='alert alert-danger'>Error menyimpan data mahasiswa: " . mysqli_stmt_error($stmt) . "</div>";
            }
...
<body>
    <div class="card-container">
        ...
        <form method="post">
            <div class="mb-3">
                <label for="nim_input">NIM</label>
                <input type="text" name="nim" id="nim_input" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="nama_input">Nama</label>
                <input type="text" name="nama" id="nama_input" class="form-control" required>
            </div>
            <div class="mb-3">
                <label for="alamat_input">Alamat</label>
                <textarea name="alamat" id="alamat_input" class="form-control" required></textarea>
            </div>
            <div class="mb-3">
                <label for="tanggal_lahir_input">Tanggal Lahir</label>
                <input type="date" name="tanggal_lahir" id="tanggal_lahir_input" class="form-control" required>
            </div>
